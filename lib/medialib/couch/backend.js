...
Backend.prototype.createRecord = function( record, whenCreatedCallback ) {
   
   var self = this;
   var saveRecord = function( callback ) {
      self.db.save( record, callback );
   };

   function whenWaterfallFinished( err, couchResponse ) {

      record._id = couchResponse.id;
      record._rev = couchResponse.rev;
     
      whenCreatedCallback( err, record._id );
   };

   async.waterfall( [saveRecord], whenWaterfallFinished );     
};
...
Backend.prototype.saveRecord = function( record, whenSavedCallback ) {
   this.db.save( record, whenSavedCallback );
}; 

//////////////////////////////////////////////////////////////////////////////////////////////////////////////

Backend.prototype.getRecord = function( id, whenRetrievedCallback ) {
   this.db.get( id, function( err, couchResponse ) {
      whenRetrievedCallback( err, couchResponse );
   } );
};
