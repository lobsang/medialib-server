...
MediaLibrary.prototype.linkifySong = function( record, onComplete ) {
   ...
   var library = this;
   // TODO: make Select bijective (from media -> mp3)
   var mediaLinks = Enumerable.From( record.entity.links )
      .Where( function( link ) { return link.rel === 'mp3' && link.url.toLowerCase().indexOf( 'file://' ) === 0; } )
      .Select( function( link ) { return { rel: 'media', url: library.baseURL + '/play/' + record.entityID + '?source=file' }; } )
      .ToArray();
