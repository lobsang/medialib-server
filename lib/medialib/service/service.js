...
MediaLibraryService.prototype.start = function( whenStartedCallback ) {
   ...
   resourceMappings[ libraryPath ].mediaFiles.addAffordance( 'get', 'play', function( request, response ) {
      var mediaFile = request.params.mediaFile;
      var options = request.query;
      
      res.contentType( options.mediaType );
      self.library.playStream( mediaFile, options, response );   
   } );
...
function createRequestContext( request, resourceMappings, mediaLibrary )
{
   ...
   return {
      ...
      affordancesLinks: function() {
         ...
         return contextNode.affordances.map( function( affordance ) {
            var rel = affordance.name;
            
            return { rel: rel, url: prefixURL + '/' + affordance.path };
         } );
