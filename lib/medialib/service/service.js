...
MediaLibraryService.prototype.start = function( whenStartedCallback ) {
   ...
   this.service.use( function( req, res, next ) {
      req.context = createRequestContext( req, self.mediaLibrary );
   ...
   routingTable.root[ libraryPath ].
      add( 'songs', resources.resourceActions( entities.types.Song ) ).
      add( 'artists', resources.resourceActions( entities.types.Artist ) );   

   routingTable.root[ libraryPath ].songs.
      add( 'mediaFiles', resources.resourceActions( entities.types.MediaFile ) ).
      add( 'artists', resources.resourceActions( entities.types.Artist ) );
...
function createRequestContext( request, mediaLibrary )
{
   ...
   return {
      ...
      resourceSelfLink: function( entity ) {
         return { rel: 'self', url: resourceURL( entity ) };
      },
