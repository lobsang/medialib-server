/**
 * Simple code by contract helpers. 
 * 
 * @example
 * 
 * Assert.that( undefined, null ).is( undefined ) // -> Error( 'null is not undefined' )
 * Assert.that( { key: 'value' } ).isPlainObject().hasKeys( 'key' ) // fine
 */
...
var tests = {
   is: function( messages, object, other ) {
      if( object !== other ) {
         messages.push( util.format( '%s !== %s', object, other ) );
         return false;
      }
