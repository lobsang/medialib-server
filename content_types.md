...
TODO: fix all links to urn style ids

All relationships between Artists, Albums and Songs are expressed bi-directionally within the resource
representations. However, physically the associations are realized as follows.
...
This means that a Song resource does not have a physical back reference to either artist or album. From this
follows that before a Song can be created, the owning Artist must already exist.

In all examples, headers which are not pertinent to that example are omitted (mostly Content-Length).
